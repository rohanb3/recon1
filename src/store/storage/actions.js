...
import {
  ...
  START_SYNC_ORDERS,
  POLLING_ORDER_SYNC,
  SYNC_ORDERS_DISPUTES,
  START_SYNC_ORDERS_DISPUTES,
  POLLING_ORDER_SYNC_DISPUTES,
} from './actionTypes';
...
export default {
  ...
  async [SYNC_ORDERS_DISPUTES]({ commit, dispatch }, dateRange) {
    commit(SET_SYNC_ORDERS_STATUS_DISPUTES, ORDER_SYNC_STATUS.WORKING);
    try {
      if (!syncOrdersIntervalId) {
        const taskId = await orderSync(dateRange);
        syncOrdersIntervalId = setInterval(() => {
      ...
      commit(SET_SYNC_ORDERS_STATUS_DISPUTES, ORDER_SYNC_STATUS.ERROR);
    }
  },
  async [START_SYNC_ORDERS_DISPUTES]({ dispatch }) {
    dispatch(SYNC_ORDERS, {
      syncOrderFromDate: moment
        .utc()
        .subtract(LAST_SIX_MONTHS, 'month')
  ...
  async [POLLING_ORDER_SYNC_DISPUTES]({ commit, dispatch, rootState }, taskId) {
    try {
      const status = await checkOrderSync(taskId);
      if (status === ORDER_SYNC_STATUS.FINISHED) {
        clearInterval(syncOrdersIntervalId);
        syncOrdersIntervalId = null;
        await dispatch(LOAD_ITEMS, {
          itemType: TABLE_NAMES.DISPUTES_ORDERS,
          filters: rootState.tables[TABLE_NAMES.DISPUTES_ORDERS].filters,
        });
        commit(SET_SYNC_ORDERS_STATUS_DISPUTES, ORDER_SYNC_STATUS.FINISHED);
      }
      if (status === ORDER_SYNC_STATUS.ERROR) {
        throw new Error();
      }
